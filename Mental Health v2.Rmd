...
# DELETE
#scale_fill_manual(values = c("#D16103", "#52854C", "#4E84C4"))
...
In JMP, we ran KNN for k=1 to 10 and found that k=8 produced the lowest misclassification rate within the KNN model.


```{r}
library(class)
k = round(sqrt(nrow(survey)), 0)

##run knn function
#knn_35 <- knn(trainNum, testNum, cl = trainNum$treatment, k = k) #run knn model
...
For this model, we used an experimental design table. We built the design table as a data table containing values for our tuning parameters. A tuning design table has a column for each option that you want to specify and has one or multiple rows that each represent a single Boosted Tree model design. We create a tuning design table which contains a column for each of the tuning parameters: number of layers, splits per tree, learning rate, minimum size split, row sampling rate, and column sampling rate.  For each row in the table, a Boosted Tree model is created using the specified tuning parameters. The model validation-set summaries report lists the R-Square value for each model. The Boosted Tree report shows the fit statistics for the model with the largest R-Square value. 


Below are the tuning parameters for the model selected by JMP’s algorithm, which selects the model with the highest Rsquare. Ideally, JMP would have selected the model with the lowest Misclassification rate (MR). Luckily, the model JMP chose also has a relatively low MR, which we will see in the next section once we have tested the model. 
